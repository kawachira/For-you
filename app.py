...
# --- 7. AI Decision Engine (Detailed Insight + Strict Scoring) ---
def ai_hybrid_analysis(price, ema20, ema50, ema200, rsi, macd_val, macd_sig, adx, bb_up, bb_low, 
    ...
    # --- SCORING SYSTEM (Strict Version) ---
    score = 0
    bullish_factors = []
    bearish_factors = []
    
    # --- 4. DETAILED FACTOR COLLECTION ---
    
    # A. Trend Structure (EMA) - Adjusted to be STRICT
    if not np.isnan(ema200):
        if price > ema200: 
            score += 1 # [Strict] กลับมาให้ +1 เท่าเดิม
            bullish_factors.append("ยืนเหนือ EMA 200 (เทรนด์หลักขาขึ้น)")
        else: 
            score -= 1 # [Strict] ลบ -1 เท่าเดิม
            bearish_factors.append("หลุด EMA 200 (เทรนด์หลักขาลง)")
            
    if not np.isnan(ema50):
        if price > ema50: 
            # [Strict] ไม่บวกคะแนนเพิ่ม (0) แต่แสดงผลให้รู้
            bullish_factors.append("ยืนเหนือ EMA 50 (ระยะกลางแกร่ง)")
        else: 
            # [Strict] ไม่ลบคะแนนเพิ่ม (0) แต่แสดงผล
    ...
    if not np.isnan(ema20):
        ...
        # ถ้า > EMA20 ไม่ได้บวกเพิ่มใน Logic เดิม
    ...
    # Safety Net 2: Heavy Selling (กฎรอง)

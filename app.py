...
# --- 1. ตั้งค่าหน้าเว็บ ---
st.set_page_config(page_title="AI Stock Master (Precision)", page_icon="💎", layout="wide")
...
# --- 3. Header & Form ---
st.markdown("<h1>💎 Ai<br><span style='font-size: 1.5rem; opacity: 0.7;'>Ultimate Sniper (Precision Week & Smart Vol)🚀</span></h1>", unsafe_allow_html=True)
...
if submit_btn:
    ...
    if df is not None and not df.empty and len(df) > 20:
        ...
        with c3:
            ...
            if dz: st.write(f"- Demand Zone: {dz[-1]['bottom']:.2f} - {dz[-1]['top']:.2f}")
            else: st.write("- No Demand Zone nearby")

...
st.markdown("""
    ...
    /* --- NEW ADDITION: X-Ray Box Style (ส่วนเสริม ไม่กระทบของเก่า) --- */
    .xray-box {
...
# ส่วนนี้คือส่วนสมอง ที่ถูกอัปเกรดให้ฉลาดขึ้นตามความจริง (Reality Based)
def ai_hybrid_analysis(price, ema20, ema50, ema200, rsi, macd_val, macd_sig, adx, bb_up, bb_low, 
    ...
    # เช็คทรงกราฟ (Structure) ว่าเป็นขาขึ้นไหม? (Quiet Uptrend Detection)
    ...
    # 3.1: REALITY FIX: แก้เรื่อง ADX ต่ำแต่กราฟทรงดี (Quiet Uptrend)
    if not np.isnan(adx) and adx < 25:
        if is_uptrend_structure:
            # ADX ต่ำ แต่ทรงดี = ขาขึ้นแบบค่อยเป็นค่อยไป (ไม่ปรับเป็น 0)
            situation_insight = "📈 **Quiet Uptrend:** ราคาไต่ระดับขึ้นแบบค่อยเป็นค่อยไป (Low Volatility) แม้ ADX จะต่ำแต่นี่คือขาขึ้นที่มั่นคง"
            bullish_factors.append("ราคาไต่ขึ้นต่อเนื่องในกรอบ (Creeping Trend)")
            
        elif is_big_candle and "Bullish" in candle_pattern:
            # Breakout
            score += 3 
            situation_insight = "🚀 **Awakening Breakout:** ตลาดระเบิดพลังจากความเงียบด้วยแท่งเทียนใหญ่!"
            bullish_factors.append("Breakout พ้นจากโซน Sideway")
            
        elif is_big_candle and "Bearish" in candle_pattern:
            # Breakdown
            score -= 3
            situation_insight = "💥 **Panic Breakdown:** ตลาดทิ้งตัวแรงจากความเงียบ!"
    ...
    # 3.2: Reversal Logic
        ...
        score += 2 
        situation_insight = "↩️ **Potential Reversal:** เทรนด์หลักลง แต่เกิดแท่งเทียนกลับตัว (Hammer) ในโซน Oversold"
    ...
    if is_squeeze:
        if is_big_candle:
            ...
            situation_insight = "⚡ **Volatility Squeeze:** กราฟบีบตัวแน่น เตรียมเลือกทาง"
    ...
    # 5. สรุป Strategy (ปรับ Rank ให้สมจริง)
    status_color = "yellow"
    banner_title = ""
    strategy_text = ""
    holder_advice = ""
    ...
    if is_squeeze and not is_big_candle:
        ...
        strategy_text = "Buy on Dip / Hold"
        holder_advice = f"เทรนด์ยังดีมาก ถือต่อได้ ถ้าย่อมาแถว EMA 20 ({ema20:.2f}) เป็นโอกาสสะสม"
        
    elif score >= 1:
        # ปรับให้ยอมรับ Quiet Uptrend
        status_color = "green"
        banner_title = "📈 Moderate Bullish: ขาขึ้นต่อเนื่อง"
        strategy_text = "Accumulate (ทยอยสะสม)"
